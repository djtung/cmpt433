...
#define DRUM_BD_FILE "wave-files/100051__menegass__gui-drum-bd-hard.wav"
#define DRUM_HH_FILE "wave-files/100053__menegass__gui-drum-cc.wav"
#define DRUM_SNR_FILE "wave-files/100058__menegass__gui-drum-snare-hard.wav"
...
int main(int argc, char* argv[])
{
	...
	wavedata_t combinedSample;

	combinedSample.pData = malloc(150000*SAMPLE_SIZE);
	combinedSample.numSamples = 150000;

	Audio_readWaveFileIntoMemory(DRUM_BD_FILE, &sampleFile1);
	Audio_readWaveFileIntoMemory(DRUM_HH_FILE, &sampleFile2);

	for (i = 0; i < sampleFile2.numSamples; i++) {
		combinedSample.pData[i] += sampleFile2.pData[i];
	}

	for (i = 0; i < sampleFile1.numSamples; i++) {
		combinedSample.pData[i] += sampleFile1.pData[i];
	}

	for (i = 0; i < 150000; i++) {
		if (combinedSample.pData[i] >= 32767) {
			combinedSample.pData[i] = 32767;
		}
		if (combinedSample.pData[i] <= -32767) {
			combinedSample.pData[i] = -32767;
		}
	...
	// Play Audio
	Audio_playFile(handle, &combinedSample);
	//	Audio_playFile(handle, &sampleFile);
	//	Audio_playFile(handle, &sampleFile);

	printf("%d\n", sampleFile1.numSamples);
	printf("%d\n", sampleFile2.numSamples);
